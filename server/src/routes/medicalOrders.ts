...
router.get(
  ...
  validateQuery({
    ...
    appointment_id: v.string({ min: 1, max: 50, optional: true }),
  }),
  (req: AuthRequest, res) => {
    try {
      ...
      if (appointment_id && typeof appointment_id === 'string' && appointment_id !== 'all') {
...
router.post(
  ...
  validateBody({
    orders: v.array(
      v.object({
        ...
        order_type: v.enum(['examen', 'especialista', 'laboratorio', 'procedimiento'], { optional: true, default: 'examen' }),
        description: v.string({ min: 1, max: 2000, optional: true }),
        file_url: v.string({ min: 1, max: 500, optional: true }),
        file_type: v.string({ min: 1, max: 50, optional: true }),
      }),
      { min: 1, max: 50 }
    ),
  }),
  (req: AuthRequest, res) => {
    try {
      ...
      // Validate ownership of all referenced files to prevent cross-tenant hijacking
      for (const item of orders) {
        if (item && item.file_url && typeof item.file_url === 'string' && item.file_url.trim()) {
          if (!isFileOwnedByFamily(familyId, item.file_url)) {
            return res.status(403).json({ error: 'Acceso denegado: Uno o más archivos no pertenecen a su grupo familiar.' });
          }
        }
      }
...
router.put(
  ...
  validateRequest({
    ...
    body: {
      ...
      description: v.string({ min: 1, max: 2000, optional: true }),
      order_type: v.enum(['examen', 'especialista', 'laboratorio', 'procedimiento'], { optional: true }),
      status: v.enum(['pendiente', 'agendada', 'completada'], { optional: true }),
      linked_appointment_id: v.uuid({ optional: true }),
      file_url: v.string({ min: 1, max: 500, optional: true }),
    },
  }),
  (req: AuthRequest, res) => {
    try {
      const familyId = req.family!.id;
      const { id } = req.params;
      const { title, description, status, linked_appointment_id, order_type, file_url } = req.body;
      ...
      // Validate file_url ownership if updated
      if (file_url && typeof file_url === 'string' && file_url.trim() && file_url !== existing.file_url) {
        if (!isFileOwnedByFamily(familyId, file_url)) {
          return res.status(403).json({ error: 'El archivo especificado no pertenece a su grupo familiar.' });
        }
      }
